    ...
    <section class="movie-details-area" data-background="/assets_client/img/bg/cinema2.png">
        <div id="app" class="container">
            ...
            <div class="row align-items-center position-relative">
                <div class="col-md-12">
                    <div class="alert text-center text-white" style="background-color: darkgray; font-size: 23px"
                        role="alert">
                        <b>MÀN HÌNH</b>
                    </div>
                </div>
                        ...
                        <table style="border-collapse:separate" class="table table-bordered">
                            <tr v-for="i in (0, hang_ngang)" style="cursor: pointer">
                                <template v-for="j in (0, hang_doc)">
                                    <td style="border: 1px solid darkgrey; margin: 5px"
                                        v-on:click="doiTrangThaiSangGiuCho(ds_ghe[(i - 1) * hang_doc +  j - 1])"
                                        v-if="ds_ghe[(i - 1) * hang_doc +  j - 1].trang_thai == 0"
                                        style="background-color: #e6e7e9" class="text-center align-middle">
                                        @{{ ds_ghe[(i - 1) * hang_doc + j - 1].ten_ghe }}
                                    </td>
                                    <td style="border: 1px solid darkgrey; margin: 5px"
                                        v-else-if="ds_ghe[(i - 1) * hang_doc +  j - 1].trang_thai == 1"
                                        class="bg-danger text-center align-middle text-white">
                                        @{{ ds_ghe[(i - 1) * hang_doc + j - 1].ten_ghe }}
                                    </td>
                                    <template v-else>
                                        <td style="border: 1px solid darkgrey; margin: 5px"
                                            v-on:click="doiTrangThaiSangKhongDat(ds_ghe[(i - 1) * hang_doc +  j - 1])"
                                            v-if="ds_ghe[(i - 1) * hang_doc +  j - 1].id_khach_hang == id_login"
                                            class="bg-info text-center align-middle text-white">
                                            @{{ ds_ghe[(i - 1) * hang_doc + j - 1].ten_ghe }}
                                        </td>
                                        <td style="border: 1px solid darkgrey; margin: 5px"
                                            v-on:click="doiTrangThaiSangGiuCho(ds_ghe[(i - 1) * hang_doc +  j - 1])" v-else
                                            style="background-color: #e6e7e9" class="text-center align-middle">
                                            @{{ ds_ghe[(i - 1) * hang_doc + j - 1].ten_ghe }}
                                        </td>
                                    </template>
                ...
                <div class="col-md-12 text-center pt-2" style="text-align: right">
                    <a href="/client/thanh-toan" class="m-1 banner-btn btn wow">Tiếp Tục</a>
                </div>
            </div>
        </div>
    </section>
